...
#from CompetitionTemplate.command_generator.generator import read_data, parse_objects, parse_rooms, parse_locations, parse_names
from ros_whisper_vosk.srv import SetGrammarVosk, SetGrammarVoskRequest
...
def main():
    ...
    print("GPSR.->Waiting for /set grammar vosk service")
    rospy.wait_for_service("/set_grammar_vosk", 10.0)
    print("GPSR.->Service is now available")
    clt = rospy.ServiceProxy("/set_grammar_vosk", SetGrammarVosk)
    req = SetGrammarVoskRequest()
    req.grammar = vosk_grammar
    clt(req)
    print("GPSR.->New vosk grammar set")
