...
class Initial(smach.State):
    ...
    def execute(self, userdata):
        print('\n> STATE <: INITIAL')
        print("INITIALIZING FOLLOW DEMO .......ᐠ( ᐢ ᵕ ᐢ )ᐟ")
        self.tries += 1
        ...
        gram = userdata.confirm_list + userdata.negation_list + ["justina stop follow"]
        ...
        print('I am ready to execute follow demo.')
        voice.talk('I am ready to execute follow demo.')
...
class FindHuman(smach.State):
    ...
    def execute(self, userdata):
        ...
        if self.tries == 2:
            ...
            head.set_named_target('s_right')
            rospy.sleep(0.3)
            head.set_named_target('s_right')
        if self.tries == 3:
            print("looking right =_o")
            head.set_named_target('s_left')
            rospy.sleep(0.3)
            head.set_named_target('s_left')
...
#             rospy.sleep(0.3)
#             head.set_named_target('face_to_face')
#             print("I will try to find you. Please stand in front of me")
#             voice.talk("I will try to find you. Please stand in front of me")
#             self.tries = 0
#             return 'succ'            
#         else:
#             return 'tries'
...
class FindLegs(smach.State):
    ...
    def execute(self, userdata):
        ...
        if self.tries > 6:  # TODO: many times
            print("I can't found you, please stand one meter in front of me")
            voice.talk("I can't found you, please stand one meter in front of me")
        ...
        if self.tries == 1:
            ...
            #JuskeshinoVision.enableHumanPose(True)  # TODO: try ----------------------------?
            JuskeshinoHRI.enableLegFinder(True)
            loop = rospy.Rate(10)
            max_attempts = 30
            while not JuskeshinoHRI.frontalLegsFound() and max_attempts > 0:
                loop.sleep()
                max_attempts -= 1
        ...
        if legs_found.data:
            print('I found you')
            voice.talk('I found you')
            print("Please say Justina stop follow when you want to end the following")
            voice.talk("Please say. . justina stop follow, when you want to end the following")
            JuskeshinoHRI.enableHumanFollower(True)
            print('Now you can walk')
            voice.talk('Now you can walk')
            self.tries = 0
            
            return 'succ'
        else:
            print("I can't found you, please stand closer to me")
            voice.talk("I can't found you, please stand closer to me")
    ...
    def execute(self, userdata):
        ...
        if(legs_found.data):
            ...
            if self.tries % 7 == 0:
                ...
                voice.talk("I am following you")
            

            command_voice = JuskeshinoHRI.getLastRecognizedSentence()
            print("--> command_voice: ", command_voice)
            if command_voice != None and "justina stop follow" in command_voice:
            ...
            print("I lost you, please stop and stand in front of me")
            voice.talk("I lost you, please stop and stand in front of me")
...
class AskArrive(smach.State):
    ...
    def execute(self, userdata):
        ...
        if self.tries == 1:
            ...
            print('Did you say justina stop follow?')
            voice.talk ('Did you say, justina stop follow?')
            # print('Please say: Robot yes or robot no')
            # voice.talk('Please say: Robot yes. or robot no')
            head.set_named_target('face_to_face')
            rospy.sleep(0.3)
            head.set_named_target('face_to_face')
        ...
        print('Please say: Robot yes.')
        voice.talk('Please say: Robot yes.')
        print('Or robot no')
        voice.talk('Or robot no.')
        ...
        if vosk_enable:
            ...
            JuskeshinoHRI.getLastRecognizedSentence()
            rospy.sleep(0.3)
            confirmation = JuskeshinoHRI.waitForNewSentence(2) # 10 is to much?
        ...
        if "robot yes" in confirmation: # userdata.confirm_list:
            ...
            voice.talk('Ok. ')
            rospy.sleep(0.3)
            print('Demo finish thanks for watching')
            voice.talk('Demo finish. thanks for watching')
            head.set_named_target('face_to_face')
            rospy.sleep(0.3)
            head.set_named_target('face_to_face')
            return 'succ'
...
if __name__ == '__main__':
    ...
    with sm:
        ...
        smach.StateMachine.add("FIND_HUMAN", FindHuman(), 
                               transitions={'failed':'FIND_HUMAN', 'succ':'FIND_LEGS', 'tries':'FIND_HUMAN'})
        ...
        smach.StateMachine.add("ASK_ARRIVE", AskArrive(), 
                               transitions={'failed':'FOLLOW_HUMAN', 'succ':'END', 'tries':'ASK_ARRIVE'})
