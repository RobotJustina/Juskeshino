...
class Wait_door_opened(smach.State):
    ...
    def execute(self, userdata):
        ...
        if np.mean(msg.ranges[(int)(len(msg.ranges)*0.5 -10):(int)(len(msg.ranges)*0.5 +10)]) < 1.0   :return 'failed' # DOOR
        self.tries = 0
        return 'succ'
...
class Goto_door(smach.State):  # ADD KNONW LOCATION DOOR
    ...
    def execute(self, userdata):
        ...
        if self.tries == 3:
            return 'tries'
        if self.tries == 1: 
            print ("voice.talk('Navigating to, door')")
...
class Find_sitting_place(smach.State):
    ...
    def execute(self, userdata):
        ...
        if res == None:
            ...
            trans,_  = tf_man.getTF(target_frame = place_face, ref_frame= 'base_link')
...
if __name__ == '__main__':
    ...
    with sm:
        ...
        smach.StateMachine.add("INITIAL",           Initial(),              transitions={'failed': 'INITIAL',           'succ': 'WAIT_DOOR_OPENED',   'tries': 'END'})
        smach.StateMachine.add("WAIT_DOOR_OPENED",  Wait_door_opened(),     transitions={'failed': 'WAIT_DOOR_OPENED',  'succ': 'GOTO_DOOR',        'tries': 'END'})
