data = readmatrix('Velocidades.csv');
%data = csvread('Velocidades.csv');
n_regions=8;

figure
[idx1,C1] = LBG(data,n_regions);
subplot(2,1,1);
gscatter(data(:,1),data(:,2),idx1)
hold on
plot(C1(:,1),C1(:,2),'kx')
title('LBG algorithm')

[idx2,C2] = kmeans(data,n_regions);
subplot(2,1,2);
gscatter(data(:,1),data(:,2),idx2)
hold on
plot(C2(:,1),C2(:,2),'kx')
title('k means algorithm')

writematrix(idx1,'Index.csv','Delimiter','comma')
writematrix(C1,'Centroid.csv','Delimiter','comma')
...
function[idX,C]=divide(X,C,nc,k)
    ...
    while(abs(d_ant-d_act)>0.000001 && count <= 700) %%while the tolance is greater than 0.01
