...
#define TICKS_PER_METER_FRONTAL	137635
#define TICKS_PER_METER_LATERAL 137635 
...
class EncoderOdom
{
    public:
        ...
        void update(int32_t encLeft, int32_t encRight, int32_t encRear)
        {
            ...
            float distX = -(1.732050808 * distRight - 1.732050808 * distLeft) / 3.0;
            float distY = -(distRight +  distLeft - 2.0 * distRear)   / 3.0; 
            float deltaTheta = -(distRight + distLeft + distRear) / (base_width / 2.0) / 3.0;
            float deltaX;
            float deltaY;
            ...
            // TODO Comentar si no funcionan mis calculos hehehe 
            if(fabs(deltaTheta) > 0.00001)
            {
                float rgX = distX / deltaTheta;
                float rgY = distY / deltaTheta;
                deltaX = rgX * sin(deltaTheta)          +    rgY * (1.0 - cos(deltaTheta));
                deltaY = rgX * (1.0 - cos(deltaTheta))  +    rgY * sin(deltaTheta);
            }
            else
        ...
        void updatePublish(int32_t encLeft, int32_t encRight, int32_t encRear)
        {
            ...
            if(abs(encLeft - this->lastEncLeft) < 93000 and abs(encRight - this->lastEncRight) < 93000 and abs(encRear - this->lastEncRear) < 93000)
                this->update(encLeft, encRight, encRear);
            else{
...
void checkSpeedRanges(float &sLeft, float &sRight, float &sRear)
{
    float maxValueFrontal = std::max(fabs(sLeft), fabs(sRight));
    float maxValueLateral = fabs(sRear);
...
void callbackCmdVel(const geometry_msgs::Twist::ConstPtr &msg)
{
    ...
    float linearX   = 0.866025404 *  msg->linear.x - 0.5 * msg->linear.y;
    float linearY   = -0.866025404 *  msg->linear.x - 0.5 * msg->linear.y;
    float angularZ  = msg->angular.z;
        
    leftSpeed     = linearX - angularZ * BASE_WIDTH / 2.0;
    rightSpeed    = linearY - angularZ * BASE_WIDTH / 2.0;
    rearSpeed     = msg->linear.y - angularZ * BASE_WIDTH / 2.0;

    if(!simul)
    {
        checkSpeedRanges(leftSpeed, rightSpeed, rearSpeed);
        ...
        try
        {
            rcLateral->SpeedM2(rcAddressLateral, rearSpeed); 
        }
        catch(std::exception &e)
        {
            ROS_ERROR("Mobile base.-> Error while writing speeds to roboclaw frontal:");
...
int main(int argc, char ** argv)
{
    ...
    std::string portFrontal = "/dev/ttyACM0";
    std::string portLateral = "/dev/ttyACM1";
    ...
    if(!simul)
    {
        ...
        try
        {
            ...
            rcLateral->SpeedM2(rcAddressLateral, 0);
...
*/
    ...
    std::string jointNames[4] = {"wheel_front_left_connect", "wheel_front_right_connect", "wheel_back_connect"};
    float jointPositions[4] = {0.0, 0.0, 0.0};
    sensor_msgs::JointState jointState;
    // Se asignan los nombres de los joints y las dimensiones 
    jointState.name.insert(jointState.name.begin(), jointNames, jointNames + 3);
    jointState.position.insert(jointState.position.begin(), jointPositions, jointPositions + 3);

    uint8_t statusEncLeft, statusEncRight, statusEncRear;
    bool validLeft, validRight, validRear;
    ...
    while(ros::ok())
    {
        ...
        encodm.updatePublish(encoderLeft, encoderRight, encoderRear);	
