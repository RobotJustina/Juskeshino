...
void callbackRelativeHeight(const std_msgs::Float32::ConstPtr &msg){
    ...
    goalPose.data = absPosition + msg->data;
    unsigned int goalTarget;
    if(goalPose.data < 0.01 || goalPose.data > 0.294){
        std::cout << "torso_node_pololu.->Can not reached the goal position, adjust the nearest goal reached." << std::endl;
        if(goalPose.data < 0.01f)
            goalPose.data = 0.01f;
        if(goalPose.data > 0.294)
            goalPose.data = 0.294f;
        goalSpeeds_simul[0] = 0.004;
    }
    if(!simul){
        goalTarget = getFeedbackFromPosition(goalPose.data * 100.0f);
...
void callbackAbsoluteHeight(const std_msgs::Float32::ConstPtr &msg){
    ...
    if(msg->data < 0.02 || goalPose.data > 0.29){
        std::cout << "torso_node_pololu.->Cannot reach the goal position, adjust to nearest goal reachable." << std::endl;
        if(goalPose.data < 0.02)
            goalPose.data = 0.02f;
        if(goalPose.data > 0.29)
            goalPose.data = 0.29f;
        goalSpeeds_simul[0] = 0.004;
    }
    if(!simul){
        unsigned int goalTarget = getFeedbackFromPosition(goalPose.data * 100.0f);
...
int main(int argc, char ** argv){
    ...
    while(ros::ok()){
        if(!simul){         
            ...
            deltaPose[0] = goalPose.data - positions[0];
        ...
        msgCurrentPose.data = positions[0];
