...
class MixtureDensityNetwork(nn.Module):
    ...
    """
    def __init__(self, dim_in, dim_out, n_components, hidden_dim, noise_type=NoiseType.FULL, fixed_noise_level=None):
        super().__init__()
        assert (fixed_noise_level is not None) == (noise_type is NoiseType.FIXED)
        num_sigma_channels = {
            NoiseType.FULL: dim_out * dim_out * n_components,
    ...
    def forward(self, x, eps=1e-6):
        ...
        sigma = sigma.reshape(-1, self.n_components, self.dim_out, self.dim_out)
        return log_pi, mu, sigma

    def loss(self, x, y):
        log_pi, mu, sigma = self.forward(x)
        #z_score = (y.unsqueeze(1) - mu) / sigma
        z_score2 = (y.unsqueeze(1) - mu)
        # normal_loglik = (
        #     -0.5 * torch.einsum("bij,bij->bi", z_score, z_score)
        #     -torch.sum(torch.log(sigma), dim=-1)
        # )
        fnormal_loglik = -0.5(
            torch.matmul(z_score2.T,torch.linalg.solve(sigma,z_score2))
            +torch.log(torch.det(sigma))
        )
        loglik = torch.logsumexp(log_pi + fnormal_loglik, dim=-1)
        return -loglik

    def sample(self, x):
        log_pi, mu, sigma = self.forward(x)
        print(log_pi.shape, mu.shape,sigma.shape)
        cum_pi = torch.cumsum(torch.exp(log_pi), dim=-1)
        rvs = torch.rand(len(x), 1).to(x)
        rand_pi = torch.searchsorted(cum_pi, rvs)
        print(torch.randn_like(mu).unsqueeze(-1).shape)
        rand_normal = torch.matmul(sigma, torch.randn_like(mu).unsqueeze(-1)) + mu
