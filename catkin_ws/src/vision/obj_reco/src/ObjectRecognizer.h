...
public:
    ...
    static bool train_from_folder(std::string training_folder, int histogram_size, bool debug=false);
    static bool recognize(cv::Mat img_bgr, cv::Mat mask, int hist_size, std::string& recognized, double& similarity,bool debug=false);
};
