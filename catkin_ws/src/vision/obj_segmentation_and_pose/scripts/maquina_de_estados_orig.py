...
def position():
    ...
    return result_recog_obj.recog_object
...
def main():
    ...
    # E:posicion de objetos respecto a 'base_link'
        reco_object = position()
        while (reco_object.pose.position.x == 0) or (reco_object.pose.position.y == 0) or (reco_object.pose.position.z < 0) or (reco_object.pose.position.x == 'nan'): 
            print("Recalculando la posicion....................")
            reco_object = position()

        # Orientacion y posicion del objeto detectado:
        x = reco_object.pose.position.x
        y = reco_object.pose.position.y
        z = reco_object.pose.position.z
        X = reco_object.pose.orientation.x
        Y = reco_object.pose.orientation.y
        Z = reco_object.pose.orientation.z
        W = reco_object.pose.orientation.w

        print("Pose obje", reco_object.pose)

    
        R,P,Y = tft.euler_from_quaternion([X,Y,Z,W]) # conversion a quaternion
        print("euler from quaternion...............")
    
        
    
    # servicio de mejor pose de gripper *********************************************
        ...
        Pose2traj_msg.yaw = Y
        Pose2traj_msg.initial_guess = [float(reco_object.category)]
