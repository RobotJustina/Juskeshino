...
struct SkeletonKeypoint
{
    cv::Point point;
    float conf;
    int idx;
    SkeletonKeypoint(cv::Point point,float conf,int idx):
...
Skeleton(std::vector<cv::Point> keypoints, float confidence=1):
    ...
    Skeleton& operator=(const Skeleton& obj){
...
{
public:
    void track(std::vector<Skeleton> &current_poses, int threshold = 3){
        std::sort(current_poses.begin(), current_poses.end(),
                  [](const Skeleton& a, const Skeleton& b){
...
private:
    int max_pose_id = -1;
    std::vector<Skeleton> previous_poses;
    ...
    int get_similarity(const Skeleton& a, const Skeleton& b, float threshold = 0.5){
        ...
        for(int kpt_id = 0; kpt_id < Skeleton::num_kpts; kpt_id++){
            if(a.keypoints[kpt_id].x != -1 && b.keypoints[kpt_id].x != -1){
                ...
                double similarity = exp( -distance / (2 * area) * SkeletonTracker::vars[kpt_id]);
