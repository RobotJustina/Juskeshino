...
public:
    SkeletonDetector(std::string modelPath){
        this->net = cv::dnn::readNetFromONNX(modelPath);
        this->net.setPreferableBackend(cv::dnn::DNN_BACKEND_CUDA);
        this->net.setPreferableTarget(cv::dnn::DNN_TARGET_CUDA);
    }

    std::vector<Skeleton> run(const cv::Mat& img, const bool multi_person = false){
        std::vector<cv::Mat> outs = this->forward(img);
        std::vector<Skeleton> res = this->postProcess(outs);
...
private:
    ...
    std::vector<Skeleton> postProcess(const std::vector<cv::Mat>& outs){
        ...
        std::vector<std::vector<SkeletonKeypoint>> all_keypoints_by_type;
        ...
        cv::Mat pose_entries;
        std::vector<SkeletonKeypoint> all_keypoints;
        ...
        std::vector<Skeleton> current_poses;
        for(int i=0;i<pose_entries.rows;i++){
            std::vector<cv::Point> pose_keypoints(Skeleton::num_kpts,cv::Point(-1,-1));
            int valid_num = 0;
            for(int kpt_id = 0; kpt_id < Skeleton::num_kpts; kpt_id++){
                if(pose_entries.at<float>(i, kpt_id) != -1){
                    pose_keypoints[kpt_id]=all_keypoints[(int)pose_entries.at<float>(i,kpt_id)].point;
                    valid_num++;
                }
            }
            if(valid_num >= Skeleton::num_kpts/2){
                Skeleton p(pose_keypoints,pose_entries.at<float>(i,18));
    ...
    void group_keypoints(const std::vector<std::vector<SkeletonKeypoint>>& all_keypoints_by_type, const std::vector<cv::Mat> &pafs_channels, 
                         cv::Mat &filtered_entries,std::vector<SkeletonKeypoint> &all_keypoints,
                         int pose_entry_size=20, float min_paf_score=0.05)
    {
        ...
        for(int part_id = 0 ;part_id<BODY_PARTS_PAF_IDS.size();part_id++)
        {
            ...
            std::vector<SkeletonKeypoint> kpts_a = all_keypoints_by_type[BODY_PARTS_KPT_IDS[part_id][0]];
            std::vector<SkeletonKeypoint> kpts_b = all_keypoints_by_type[BODY_PARTS_KPT_IDS[part_id][1]];
            ...
            if(part_id==0){
                ...
                for(int i=0;i<connections_len;i++){
                    ...
                    pose_entries.at<float>(i, pose_entries.cols - 2) = all_keypoints[kpts_a[a_idx[i]].idx].conf +
                        all_keypoints[kpts_b[b_idx[i]].idx].conf + affinity_scores_valid_limbs_vector[i];
                }
            }
                ...
                for(int i=0;i<connections_len;i++){
                    ...
                    if(num==0){
                        ...
                        pose_entry.at<float>(0, pose_entry.cols - 2) = all_keypoints[kpts_a[a_idx[i]].idx].conf + all_keypoints[kpts_b[b_idx[i]].idx].conf +
                            affinity_scores_valid_limbs_vector[i];
    ...
    int extract_keypoints(cv::Mat& heatmap, std::vector<std::vector<SkeletonKeypoint>>& all_keypoints, int& total_keypoint_num)
    {
        ...
        std::vector<SkeletonKeypoint> keypoints_with_score_and_id;
        int keypoint_num = 0;
