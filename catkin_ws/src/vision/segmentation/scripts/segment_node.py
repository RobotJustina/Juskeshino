...
def trigger_response(request):
    ...
    if request.height.data == -1:
        ...
        print(
            f'Number of planes found {len(inds[0])} at z=[{bins[ np.add(inds, 1)]}]#############3')

        if (low_planes_height[0] > -0.05) and (low_planes_height[0] < 0.05):
            low_planes_height = low_planes_height[1:]
    else:
        # HEIGHT REQUESTED OR OBTAINED FROM HISTOGRAM
        low_plane = (corrected['z'] > request.height.data)
        low_planes_height = []
        if request.height.data == 0:
            low_planes_height.append(request.height.data - 0.075)
        else:
    ...
    for low_planes_h in low_planes_height:
        ...
        # HEIGHT REQUESTED OR OBTAINED FROM HISTOGRAM
        low_plane = (corrected['z'] > low_planes_h)
        ...
        cv2_image = cv2.cvtColor(
            binary_image.astype(np.uint8), cv2.COLOR_GRAY2BGR)
        img = cv2.bitwise_and(orig_image, cv2_image)
        image_with_contours = img.copy()
        #######################################
        contours, hierarchy = cv2.findContours(binary_image.astype(
            np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        if len(contours) == 0:
            print(f'no objs at z={request.height.data} plane')
        for contour in contours:
            area = cv2.contourArea(contour)
            # AREA IN PIXELS ( USEFUL TO AVOID WALLS OR OTHER BIG CLUSTRS)  ( VALUES READ FROM YAML USE SEGMENTATOR TUNER)
            if area > lower_v and area < higher_v:
                # Draw contours on the image
                M = cv2.moments(contour)
                cX = int(M["m10"] / M["m00"])
                cY = int(M["m01"] / M["m00"])
                # REJECT CENTROID OUTSIDE OF THIS RANGE: READ FROM YAML
                if (cY > reg_ly_v and cY < reg_hy_v):
                    boundRect = cv2.boundingRect(contour)
                    mask = np.zeros_like(binary_image)
                    mask = cv2.rectangle(mask, (boundRect[0], boundRect[1]), (
                        boundRect[0]+boundRect[2], boundRect[1]+boundRect[3]), (255, 255, 255), -1)
                    individual_mask = (mask*binary_image).astype(np.uint8)
                    cent = np.asarray(((np.nanmean(corrected['x'][np.where(individual_mask == 1)]), np.nanmean(
                        corrected['y'][np.where(individual_mask == 1)]), np.nanmean(corrected['z'][np.where(individual_mask == 1)]))))
                    cents.append(cent)
                    # PCA
                    points_c = np.asarray((corrected['x'][np.where(individual_mask == 1)], corrected['y'][np.where(
                        individual_mask == 1)], corrected['z'][np.where(individual_mask == 1)]))

                    heights.append(max(corrected['z'][np.where(
                        individual_mask == 1)]) - min(corrected['z'][np.where(individual_mask == 1)]))
                    widths.append(max(corrected['y'][np.where(
                        individual_mask == 1)]) - min(corrected['y'][np.where(individual_mask == 1)]))
                    print('Estimated Height #######################', max(corrected['z'][np.where(
                        individual_mask == 1)]) - min(corrected['z'][np.where(individual_mask == 1)]))
                    print('Estimated Width  #######################', max(corrected['y'][np.where(
                        individual_mask == 1)]) - min(corrected['y'][np.where(individual_mask == 1)]))
                    print('Estimated Depth  #######################', max(corrected['x'][np.where(
                        individual_mask == 1)]) - min(corrected['x'][np.where(individual_mask == 1)]))
                    E_R = points_to_PCA(points_c.transpose())
                    e_ER = tf.transformations.euler_from_matrix(E_R)
                    quat = tf. transformations.quaternion_from_euler(
                        e_ER[0], e_ER[1], e_ER[2])
                    quats_pca.append(quat)
                    print(np.rad2deg(tf.transformations.euler_from_matrix(E_R)))
                    # FOR DEBUG IMAGE
                    # -1 draws all contours
                    cv2.drawContours(image_with_contours,
                                     contour, -1, (0, 255, 0), 2)
                    rgb_image = cv2.rectangle(image_with_contours, (boundRect[0], boundRect[1]), (
                        boundRect[0]+boundRect[2], boundRect[1]+boundRect[3]), (255, 255, 0), 2)
                else:
                    print(
                        f'Centroid_y out of range {cY} ,{reg_ly_v},{reg_hy_v}')
            else:
                print(
                    f'Area of contour outside of range {area} ,{lower_v},{higher_v}')
