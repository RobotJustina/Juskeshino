...
public:
    ...
    static bool debug;
    static Eigen::Affine3d get_eigen_transform(std::string dst_frame, std::string src_frame, tf::TransformListener* tf_listener);
    static void pointcloud_msg2_cv_mat(sensor_msgs::PointCloud2& pc_msg, cv::Mat& bgr_dest, cv::Mat& pc_dest);
    static void pointcloud_msg2_cv_mat(const sensor_msgs::PointCloud2::ConstPtr& pc_msg, cv::Mat& bgr_dest, cv::Mat& pc_dest);
