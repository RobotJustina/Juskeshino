...
Eigen::Affine3d Utils::get_eigen_transform(std::string dst_frame, std::string src_frame, tf::TransformListener* tf_listener)
{
    tf::StampedTransform tf;
    tf_listener->waitForTransform(dst_frame, src_frame, ros::Time(0), ros::Duration(0.5));
    tf_listener->lookupTransform(dst_frame, src_frame, ros::Time(0), tf);
    Eigen::Affine3d e;
...
void Utils::pointcloud_msg2_cv_mat(const sensor_msgs::PointCloud2::ConstPtr& pc_msg, cv::Mat& bgr_dest, cv::Mat& pc_dest)
{
    int offset_x = 0;
    int offset_y = 4;
    int offset_z = 8;
