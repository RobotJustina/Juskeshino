...
void get_robot_position_wrt_odom(tf::TransformListener& tf_listener, float& robot_x, float& robot_y, float& robot_t, std::string& target_frame)
{
    tf::StampedTransform transform;
    tf_listener.lookupTransform(target_frame, base_link_name, ros::Time(0), transform);
    robot_x = transform.getOrigin().x();
    robot_y = transform.getOrigin().y();
    tf::Quaternion q = transform.getRotation();
    robot_t = atan2((float)q.z(), (float)q.w()) * 2;
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "simple_move_odom");
    ros::NodeHandle n("~");
    ...
    std::cout << "SimpleMove.->target frame: " << target_frame << std::endl;
