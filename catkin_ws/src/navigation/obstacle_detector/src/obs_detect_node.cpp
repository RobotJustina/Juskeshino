...
bool check_collision_risk_with_cloud(sensor_msgs::PointCloud2::Ptr msg)
{
...
bool check_collision_risk_with_lidar(sensor_msgs::LaserScan::Ptr msg)
{
    if(current_speed_linear <= 0 && !debug) return false;
    ...
    Eigen::Affine3d tf = get_transform_to_basefootprint(msg->header.frame_id);
    for(size_t i=0; i < msg->ranges.size(); i+=lidar_downsampling, angle+=i*msg->angle_increment)
    {
...
void callback_lidar(sensor_msgs::LaserScan::Ptr msg)
{
    ...
    collision_risk_lidar = check_collision_risk_with_lidar(msg);
}

void callback_point_cloud(sensor_msgs::PointCloud2::Ptr msg)
{
    no_data_cloud_counter = 0;
    collision_risk_cloud = check_collision_risk_with_cloud(msg);
}
